...
<head>
    ...
    <meta name="description"
        content="{{ $metaDescription ?? 'Floristería artesanal con arreglos únicos. Flores frescas, entrega a domicilio y tarjetas personalizadas.' }}">
...
<body class="min-h-dvh bg-white text-dark font-inter" x-data="{ 
    ...
    @if(isset($promoBanner) && $promoBanner)
        <div x-data="{ dismissed: false }" x-show="!dismissed" x-transition:leave="transition ease-in duration-200"
            ...
            <div class="container mx-auto flex items-center justify-center gap-2">
                <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"
                        d="M12 8v13m0-13H8.5a2.5 2.5 0 110-5H12m0 5h3.5a2.5 2.5 0 110-5H12m0 5h8.5a1.5 1.5 0 011.5 1.5V20a1 1 0 01-1 1H3a1 1 0 01-1-1V9.5A1.5 1.5 0 013.5 8H12z" />
                </svg>
            ...
            @if($promoBanner->is_dismissible)
                <button @click="dismissed = true" class="absolute right-4 top-1/2 -translate-y-1/2 hover:opacity-70"
    ...
    <div x-show="externalModalOpen" x-cloak class="fixed inset-0 z-50 flex items-center justify-center"
        ...
        <div class="relative mx-4 w-full max-w-md rounded-2xl bg-white p-6 shadow-xl">
            ...
            <div class="flex gap-3 justify-center">
                <button class="px-4 py-2 rounded-full text-sm text-gray-600 hover:text-dark"
                    @click="externalModalOpen = false">
                    Cancelar
                </button>
                <button class="px-5 py-2 rounded-full text-sm bg-primary text-white hover:bg-primary-dark"
                    @click="window.open(externalLinkUrl, '_blank', 'noopener'); externalModalOpen = false;">
                    Continuar
                </button>
            </div>
        </div>
    ...
    <div x-data="toastNotifications()" @toast.window="add($event.detail)"
